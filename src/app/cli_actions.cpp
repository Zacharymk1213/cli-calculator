...
std::optional<int> handleCommandLine(int argc, char **argv) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--output") {
      ...
      if (format == "json") {
        ...
        std::cerr << RED << "Error: unsupported output format: " << format
                  << RESET << '\n';
        return 1;
      }
      ++i;
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (arg == "--batch" || arg == "--batch-file" || arg == "-b") {
      if (i + 1 >= argc) {
        ...
        return processBatchFileInteractive(outputFormat, batchState);
      }
      BatchState batchState;
    ...
    if (arg == "--repl" || arg == "-r") {
      if (outputFormat != OutputFormat::Text) {
        printStructuredError(std::cerr, outputFormat, "repl",
    ...
    if (!arg.empty()) {
      if (outputFormat == OutputFormat::Text) {
        ...
        printStructuredError(std::cerr, outputFormat, "unknown-argument",
  ...
  if (outputFormat != OutputFormat::Text && hasNonColorArgs) {
    printStructuredError(std::cerr, outputFormat, "output",
