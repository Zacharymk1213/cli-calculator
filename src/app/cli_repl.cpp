...
enum class CommandOrigin { Colon, LongOption, ShortOption };
...
bool readLineWithArrowsPosix(const std::string &prompt,
...
bool readLineWithHistoryPrompt(const std::string &prompt,
                               const std::vector<std::string> &history,
                               std::string &line) {
#ifndef _WIN32
...
std::optional<RawCommand> extractRawCommand(const std::string &input) {
  ...
  if (input[0] == ':') {
    ...
    std::string command = (delimiter == std::string::npos)
                              ? payload
                              : payload.substr(0, delimiter);
    std::string args = (delimiter == std::string::npos)
                           ? ""
                           : trim(payload.substr(delimiter + 1));
  ...
  if (input.rfind("--", 0) == 0) {
    ...
    std::string command = (delimiter == std::string::npos)
                              ? input.substr(2)
                              : input.substr(2, delimiter - 2);
    std::string args = (delimiter == std::string::npos)
                           ? ""
                           : trim(input.substr(delimiter + 1));
    return RawCommand{CommandOrigin::LongOption, command, args};
  }

  static const std::vector<std::string> SHORT_PREFIXES = {
      "-e", "-sqrt", "-d", "-c", "-pf", "-v", "-b", "-h", "-nc"};
  for (const auto &prefix : SHORT_PREFIXES) {
    if (input.rfind(prefix, 0) == 0) {
      std::size_t len = prefix.size();
      if (input.size() == len ||
          std::isspace(static_cast<unsigned char>(input[len]))) {
...
std::optional<ParsedCommand> interpretCommand(const RawCommand &raw) {
  ...
  if (raw.origin == CommandOrigin::Colon) {
    ...
    if (canonical == "cli-help" || canonical == "clihelp" ||
        canonical == "commands") {
  ...
  if (canonical == "square-root" || canonical == "squareroot" ||
      canonical == "sqrt") {
  ...
  if (canonical == "batch" || canonical == "batch-file" ||
      canonical == "batchfile" || canonical == "b") {
...
void appendHistoryEntry(std::vector<std::string> &history,
  ...
  if (history.size() > MAX_HISTORY_ENTRIES) {
    ...
    history.erase(history.begin(),
                  history.begin() + static_cast<std::ptrdiff_t>(toErase));
  }
}
...
recallHistoryEntry(const std::vector<std::string> &history,
...
void printHelp() {
  ...
  std::cout << "  Type expressions directly (or use ':eval <expr>'/--eval) to "
               "evaluate them.\n";
  std::cout << "  ':history' shows stored entries, '!<n>' replays a line, "
               "'exit'/'quit' leaves the REPL.\n";
  std::cout << "  Use the Up/Down arrow keys to browse command history just "
               "like in Bash.\n";
  std::cout << "  Every CLI flag works here via ':command' or '--command' "
               "(divisors, convert, square-root,\n";
  std::cout << "    prime-factorization, version, variables/list-variables, "
               "batch, help/cli-help, etc.).\n";
  std::cout << "  ':cli-help' or '--help' prints the full CLI help text; "
               "':no-color' toggles plain output.\n";
  std::cout
      << "  Structured formats (--output) are unavailable inside the REPL."
      << '\n';
}
} // namespace
...
int runRepl(OutputFormat outputFormat) {
  ...
  std::cout
      << BOLD << BLUE
      << "Starting REPL mode. Type ':help' for commands and 'exit' to leave."
      << RESET << '\n';
  while (true) {
    std::string input;
    ...
    if (auto rawCommand = extractRawCommand(trimmed)) {
      if (auto parsed = interpretCommand(*rawCommand)) {
        if (parsed->kind != CommandKind::History &&
            parsed->kind != CommandKind::ReplHelp) {
        ...
        case CommandKind::Eval:
          if (parsed->textArgument.empty()) {
            std::cout << YELLOW
                      << "Usage: :eval <expression> or --eval <expression>"
                      << RESET << '\n';
            break;
          }
        ...
        case CommandKind::Convert:
          if (parsed->args.size() != 3) {
            std::cout << YELLOW << "Usage: :convert <from> <to> <value>"
                      << RESET << '\n';
            break;
          }
        ...
        case CommandKind::Batch:
          if (parsed->args.size() > 1) {
            ...
            if (parsed->args.empty()) {
              (void)processBatchFileInteractive(OutputFormat::Text, state);
            } else {
              (void)processBatchFile(parsed->args.front(), OutputFormat::Text,
                                     state);
            }
            if (state.lastResult) {
        ...
        case CommandKind::NoColor:
          ...
          std::cout << YELLOW << "Disabled colored output for this session."
                    << RESET << '\n';
          break;
        case CommandKind::OutputUnsupported:
          std::cout
              << YELLOW
              << "Structured output (--output) isn't available in REPL mode."
              << RESET << '\n';
          break;
